...
class StreamingAgent:
    def __init__(
        ...
        self._tools_by_name: dict[str, Tool[Any]] = {tool.name: tool for tool in tools}
    ...
    def _run_until_end_turn(self) -> Iterator[StreamEvent]:
        ...
        while True:
            container_arg = self._container_id if self._container_id is not None else omit
            with self._client.messages.stream(
                ...
                for event in stream:
                    if event.type == "content_block_delta" and event.delta.type == "text_delta":
                        ...
                        or event.type == "content_block_start"
